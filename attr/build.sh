...
if [[ $1 == *-DENABLE_INSTALL_PRIVATE* ]];then
    BUILD_TYPE_ARGS="--prefix=$VKIT_PREBUILT_PRIVATE_DIR --enable-shared=no --enable-static=yes --with-pic=yes"
else
    BUILD_TYPE_ARGS="--prefix=$VKIT_PREBUILT_DIR --enable-shared=yes --enable-static=no"
fi
...
if [ "$VKIT_PLATFORM" = "linux-x86_64" ] || [ "$VKIT_PLATFORM" = "freebsd-x86_64" ] || [ "$VKIT_PLATFORM" = "freebsd-amd64" ];then
    ...
    $SHELL_DIR/configure -q --enable-silent-rules \
    ...
    $SHELL_DIR/configure -q --enable-silent-rules --host=aarch64-linux \
    $(echo $BUILD_TYPE_ARGS)
    make -j$BUILD_CPU_CORE -s
    make install > /dev/null
else
