...
protected:
    void onSidebandReply(CBaseJob::Ptr &msg_ref)
    {
        ...
        switch (msg->code())
        {
            ...
            {
                FdbMsgEventCache event_tbl;
                CFdbParcelableParser parser(event_tbl);
                if (!msg->deserialize(parser))
                {
                    fprintf(stderr, "CEventFetcher: unable to decode FdbMsgHostAddressList.\n");
                    quit();
                }
    ...
    void onOnline(const CFdbOnlineInfo &info)
    {
        if (this->isPrimary())
        {
            ...
            {
                this->invokeSideband(FDB_SIDEBAND_QUERY_EVT_CACHE, 0, info.mQOS);
            }
            else
            {
                ...
                obj->invokeSideband(FDB_SIDEBAND_QUERY_EVT_CACHE, 0, info.mQOS);
            }
        }
    }
...
private:
    ...
    void printEvents(FdbMsgEventCache &event_tbl)
    {
