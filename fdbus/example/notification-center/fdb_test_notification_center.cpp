...
int main(int argc, char **argv)
{
    if(argc < 3)
    {
        std::cout << "Usage: fdbntfcentertest 0/1 start_event[ server-name]" << std::endl;
        std::cout << "       0: subscriber; 1: publisher" << std::endl;
        return -1;
    }

    is_publisher = !!atoi(argv[1]);
    event_id_start = atoi(argv[2]);
    const char *ntf_center = FDB_NOTIFICATION_CENTER_NAME;
    if (argc >= 4)
    {
        ntf_center = argv[3];
    }
    ...
    std::string server_name = ntf_center;
    ...
    client->enableEventCache(true); // test event cache at client side
    client->publishEvents(); // the events are cached even though not connected
