...
private:
    ...
    void onRadioClientOnline(CFdbBaseObject *obj, const CFdbOnlineInfo &info, bool online)
    {
        FDB_LOG_I("component %s, endpoint %s: client online %d %d %d\n",
                    name().c_str(), obj->name().c_str(), info.mSid, online, info.mFirstOrLast);
    }
    void onRadioServerOnline(CFdbBaseObject *obj, const CFdbOnlineInfo &info, bool online)
    {
