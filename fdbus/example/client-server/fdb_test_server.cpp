...
public:
    ...
    void broadcastElapseTime(CMethodLoopTimer<CMediaServer> *timer)
    {
        ...
        et.set_minute(10);
        et.set_second(35);
        /* broadcast elapse time; "my_topic" have no mean but test */
        CFdbProtoMsgBuilder builder(et);
        broadcast(NTF_ELAPSE_TIME, builder, "my_topic");
        }
...
protected:
    ...
    void onSubscribe(CBaseJob::Ptr &msg_ref)
    {
        ...
        FDB_BEGIN_FOREACH_SIGNAL(msg, sub_item)
        {
            ...
            {
                FDB_LOG_I("group message %d, topic %s of session %d is registered! sender: %s\n\n", msg_code, topic, sid, msg->senderName().c_str());
