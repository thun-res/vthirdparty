...
#else
    FDB_LOG_I("OBJ %d , client->server: %u, arrive->reply: %u, reply->receive: %u, total: %u\n",
                obj_id, (uint32_t)time_c2s, (uint32_t)time_s2r, (uint32_t)time_r2c, (uint32_t)time_total);
#endif
}
...
protected:
    ...
    void onBroadcast(CBaseJob::Ptr &msg_ref)
    {
        ...
        FDB_LOG_I("Broadcast is received: %d; topic: %s, qos: %d\n", msg->code(), msg->topic().c_str(), msg->qos());
