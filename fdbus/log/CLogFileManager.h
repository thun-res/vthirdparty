...
public:
    ...
    bool setStorageSize(int64_t max_storage_size = mDefaultMaxStorageSize,
                        int64_t max_file_size = mDefaultMaxFileSize);
    // file size:
    //  > 0 or == 0: sync content to disk; close file; add file to cache list
    //      if > 0: using the given size as file size;
    //      if == 0: retrieve file size internally.
    //  > 0: sync content to disk; close file; do not add file to cache list
    void sync(int64_t file_size = 0);
    void stop();
    void start();
private:
    struct CFileInfo
    ...
    void checkFileSizeUncomp();
    void checkFileSizeComp();
