...
CBaseClient *CJniAFComponent::createClient(CBaseWorker *worker)
{
    return FDB_createJniClient(mName.c_str());
}

CBaseServer *CJniAFComponent::createServer(CBaseWorker *worker)
{
    ...
    if (conn_callback)
    {
        ...
        native_handle = (jlong)component->queryService(c_name, evt_tbl, [callback](
            CFdbBaseObject *obj, const CFdbOnlineInfo &info, bool is_online) {
                CGlobalParam::callConnection(callback, info, is_online);
            }
        );
    }
    else
