...
public class MediaServer
{
    private class FdbusMediaServer extends TimerTask implements FdbusServerListener
    {
        ...
        {
            System.out.println(mServer.endpointName() + ": onOnline is received: " + is_first + ", qos: " + qos);
        }
        
        public void onOffline(int sid, boolean is_last, int qos)
        {
            System.out.println(mServer.endpointName() + ": onOffline is received: " + is_last + ", qos: " + qos);
        }
        ...
        public void run()
        {
            ...
            NFdbExample.ElapseTime et = proto_builder.build();
            FdbusProtoBuilder builder = new FdbusProtoBuilder(et);
            mServer.broadcast(NFdbExample.FdbMediaSvcMsgId.NTF_ELAPSE_TIME_VALUE, "my_topic", builder);
            mTimerRunning = false;
        }
    }
