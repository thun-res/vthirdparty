...
class MyTestServer(fdbus.FdbusServer):
    ...
    def onOnline(self, sid, is_first, qos):
        print('MyTestServer: onOnline, sid: ', sid, ', first: ', is_first, ', qos: ', qos)

    def onOffline(self, sid, is_last, qos):
        print('MyTestServer: onOffline, sid: ', sid, ', last: ', is_last, ', qos: ', qos)
...
while True:
    for i in range(nr_servers):
        ...
        server_list[i].broadcast(ex.NTF_MEDIAPLAYER_CREATED,
                                 fdbus.castToCChar('hello, world!'),
                                 'topic-1')
        server_list[i].broadcast(ex.NTF_MEDIAPLAYER_CREATED,
                                 fdbus.castToCChar('good morning!'),
                                 'topic-2')
