...
static void fdb_event_handle(fdb_message_t *msg)
{
    int32_t i;
    ...
    {
        FDB_LOG_D("    data received: %d, user data: %s\n", msg->msg_data[i], (char *)msg->user_data);
    }
}

static void fdb_connection_handle(FdbSessionId_t sid, fdb_bool_t is_online,
                                    fdb_bool_t first_or_last, void *user_data, enum EFdbQOS qos)
{
    FDB_LOG_D("connection status changed- sid: %d, is_online: %d, first_or_last: %d, user dat: %s, qos: %d\n",
                sid, is_online, first_or_last, (char*)user_data, qos);
}
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        for (i = 0; i < (argc - 1); ++i)
        {
            fdb_client_invoke_async(client_array[i], FDB_TEST_MESSAGE_ID_1, buffer, Fdb_Num_Elems(buffer), 0,
                                    FDB_QOS_RELIABLE, 0, 0);
            fdb_client_invoke_async(client_array[i], FDB_TEST_MESSAGE_ID_2, buffer, Fdb_Num_Elems(buffer), 0,
                                    FDB_QOS_RELIABLE, 0, 0);
            fdb_client_invoke_async(client_array[i], FDB_TEST_MESSAGE_ID_3, buffer, Fdb_Num_Elems(buffer), 0,
                                    FDB_QOS_RELIABLE, 0, 0);
            fdb_client_invoke_async(client_array[i], FDB_TEST_MESSAGE_ID_4, buffer, Fdb_Num_Elems(buffer), 0,
                                    FDB_QOS_RELIABLE, 0, 0);
        }
        sysdep_sleep(111);
    }
